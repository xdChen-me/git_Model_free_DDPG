...
class DRLConfig:
    def __init__(self):
        ...
        self.train_eps = 150  # 训练的回合数
        self.eval_eps = 1  # 测试的回合数
        self.gamma = 0.99  # 折扣因子
        self.actor_lr = 0.0005  # 演员网络的学习率
...
def train(cfg, env, agent):
    ...
    env_name = 'LFC_area1_python_simulink_mindspore'  # 文件名
    ...
    for i_ep in range(cfg.train_eps):
        ...
        with open(cfg.train_config_path + "train_result.txt", "a") as f:
            f.write("episode: %s/%s, reward: %s, learning rate: %s\n" % (str(i_ep + 1), str(cfg.train_eps), str(ep_reward), str(cfg.actor_lr)))
